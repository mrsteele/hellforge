// Referenced from https://github.com/blizzhackers/d2data/blob/master/compile.js
 ...
 */
 ...
 files.forEach(filename => {
   ...
   let data = fs.readFileSync(inDir + filename + '.txt').toString().split(lineEnd);
   let header = data.shift().split(fieldEnd);
   let indexColumn = header.indexOf(indexes[filename]);
   let expansion = false;
   let maxKeyCount = 0;
...
//  delete full.WeaponClass;
 ...
 fs.writeFileSync(outDir + 'raw.json', JSON.stringify(full))
